...
const SigedocBajaForm = () => {
  ...
  // 2. ESTADO DEL FORMULARIO (id_tipo_tramite = 2 para BAJA)
  const [formData, setFormData] = useState({
    id_piso: "",
    id_oficina: "",
    id_sistema: 1, // SIGEDOC
    id_tipo_tramite: 2, // <--- CLAVE PARA LA BAJA
    nombre_encargado: "",
    apellido_encargado: "",
    cargo: "",
    telefono: "",
    email: "",
    // Arrancamos con un input vacío para el primer usuario
    detalles: [{ usuario_sigedoc: "" }],
  });
  ...
  // --- LÓGICA DE USUARIOS A DAR DE BAJA ---
  const agregarUsuario = () => {
    if (formData.detalles.length >= 5) {
      Swal.fire({
        icon: "warning",
        title: "Límite alcanzado",
        text: "Por seguridad, solo se pueden cargar hasta 5 bajas por formulario.",
      });
      return;
    }
    ...
    setFormData({
      ...formData,
      detalles: [...formData.detalles, { usuario_sigedoc: "" }],
    });
  };

  const eliminarUsuario = (index) => {
    const nuevosUsuarios = formData.detalles.filter((_, i) => i !== index);
    setFormData({ ...formData, detalles: nuevosUsuarios });
  };

  const handleUsuarioChange = (index, valor) => {
    const nuevosUsuarios = [...formData.detalles];
    nuevosUsuarios[index].usuario_sigedoc = valor;
    setFormData({ ...formData, detalles: nuevosUsuarios });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.detalles.length === 0) {
      Swal.fire({
        ...
        text: "Debe agregar al menos un usuario para dar de baja.",
      });
      return;
    }
    ...
    // Verificamos que no hayan dejado inputs de usuarios en blanco
    for (let i = 0; i < formData.detalles.length; i++) {
      if (!formData.detalles[i].usuario_sigedoc.trim()) {
        Swal.fire({
          icon: "error",
          title: `Error en la fila ${i + 1}`,
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `Baja_SIGEDOC_Tramite_${idTramiteGenerado}.docx`,
      );
      ...
      Swal.fire({
        ...
        html: `
          El trámite de baja se guardó y el documento se descargó con éxito.<br/><br/>
  ...
  return (
    <div className="container mt-4 mb-5 text-start">
      <button
        onClick={() => navigate("/sigedoc/baja/options")}
      ...
      <div className="card shadow-sm border-0">
        <div className="card-header bg-primary text-white py-3 text-center">
          <h4 className="m-0 fw-bold">Solicitud de Baja - SIGEDOC</h4>
        </div>
...
export default SigedocBajaForm;
